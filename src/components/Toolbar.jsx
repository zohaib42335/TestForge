...
function ProfileMenu({ profile, onSignOut }) {
  const { email, photoURL, displayName, isGoogle, role } = profile
  ...
  const roleBadge =
    role === 'Admin'
      ...
      : role === 'QA Lead'
        ...
        : role === 'Tester'
          ? 'bg-[#EEF2FB] text-[#2A4A8A] border-[#B0C0E0]'
          : null
  ...
  return (
    <div className="relative group flex items-center gap-2">
      {roleBadge ? (
        <span
          className={`hidden sm:inline-block rounded-[99px] border-[0.5px] px-[7px] py-[2px] text-[10px] font-medium leading-none ${roleBadge}`}
        >
          {role}
        </span>
      ) : null}
