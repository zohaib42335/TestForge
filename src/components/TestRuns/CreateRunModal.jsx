...
import { getTestCases } from '../../api/testCases.api.js'
import { createRun } from '../../api/testRuns.api.js'
...
export default function CreateRunModal({ projectId, testCases, testCasesLoading, onClose }) {
  ...
  const [list, setList] = useState(Array.isArray(testCases) ? testCases : [])
  ...
  const handleNext = () => {
    ...
    getTestCases(projectId, { page: 1, limit: 500, sortBy: 'testCaseId', sortOrder: 'asc' })
      .then((data) => setList(Array.isArray(data?.items) ? data.items : []))
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (created?.id) {
        // no-op; parent list refetches via hook inside RunsList
      }
      onClose()
    } catch (e) {
          ...
          <div className="px-5 pb-5">
            ...
            >
              <div className="max-h-[320px] overflow-y-auto">
                {(testCasesLoading || casesLoading) && (
                  <p className="text-[12px] text-[#5A6E9A] p-4">Loading test cases…</p>
                )}
                {!(testCasesLoading || casesLoading) && filtered.length === 0 && (
